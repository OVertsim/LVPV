...
# option "sd" cuts off all coefficients that fall outside of one standart deviation in squared matrix 
...
cor.graph = function(data, 
  ...
  if(q.cut=="sd") C[abs(C^2-mean(C^2)) < sd(C^2)] = 0
...
PV.summary<-function(x, which = NULL, digits = 3, modularity = TRUE){
  ...
  if(d){
    ...
    if(x$method[[4]]=="sd") cat('Correlations within one standart deviation 
                                   from the absolute mean cut off', "\n")
    if((is.numeric(x$method[[4]]))&(x$method[[4]]>0)) cat('Correlations cut at', 
                                                          paste(round(100*x$method[[4]],2),"%",sep=""), 
                                                          'quantile', "\n")
    if((is.numeric(x$method[[4]]))) cat('Correlations cut at', 
                                                          paste(round(100*x$method[[4]],2),"%",sep=""), 
                                                          'quantile', "\n")
...
# 
# ## use fine-tuning procedure in the leading eigenvector algorithm
# pvTD.ft = PV.complete(TestData, iseed=767, tuning = TRUE)
# PV.dendro(pvTD.ft); PV.text(pvTD.ft) 
# PV.graph.highlight(pvTD.ft, col="lightblue")
# PV.summary(pvTD.ft)
...
# method and use same as cor() function options;
# gauss allows for prior gaussian tranformation of the correlation matrix, default = FALSE;
# q.cut cuts off correlations below specified quantile (counted w/o diagonal); 
# q.cut default = NULL cuts off all coefficients that fall outside of one standart deviation in squared matrix 
...
cor.graph = function(data, 
  ...
  if(gauss|!square) C = exp(-(1-abs(C))^2/(2*sd(C)^2))
  if(is.null(q.cut)) C[abs(C^2-mean(C^2)) < sd(C^2)] = 0
  else if((0<q.cut)&(q.cut<1)) diag(C) = 0; C[C^2 < quantile(C^2, q.cut)] = 0
  if(diag) diag(C)=1
  g = graph.adjacency(C, weighted = TRUE, diag = diag, mode = 'undirected')
  if(tuning){
...
PV.complete<- function(data, nboot = 1000, 
                       r = seq(.5,1.4,by=.1),
                       method="pearson", use="everything",
                       warnings=FALSE, gauss=FALSE, square=TRUE, q.cut=NULL, diag=FALSE,
                       iseed=NULL, quiet=FALSE, tuning = FALSE, store=FALSE,
                       mod.alpha = .05){
...
PV.summary<-function(x, which = NULL, digits = 3, modularity = TRUE){
  ...
  if(d){
    ...
    if(!x$method[[6]]) cat('Negative correlations used',"\n")
    if(x$method[[3]]) cat('Gauss transformation used',"\n")
    if(is.null(x$method[[4]])) cat('Correlations within one standart deviation 
                                   from the absolute mean cut off', "\n")
    if((is.numeric(x$method[[4]]))) cat('Correlations cut at', 
                                                          paste(round(100*x$method[[4]],2),"%",sep=""), 
                                                          'quantile', "\n")
